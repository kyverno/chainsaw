...
func (p *stepProcessor) applyOperation(ctx context.Context, op v1alpha1.Apply, to *metav1.Duration) ([]operation, error) {
	...
	for _, resource := range resources {
		...
		ops = append(ops, operation{
			timeout:   timeout.Get(timeout.DefaultApplyTimeout, p.config.Timeouts.Apply, p.test.Spec.Timeouts.Apply, p.step.TestStepSpec.Timeouts.Apply, to),
...
func (p *stepProcessor) assertOperation(ctx context.Context, op v1alpha1.Assert, to *metav1.Duration) ([]operation, error) {
	...
	for _, resource := range resources {
		ops = append(ops, operation{
			timeout:         timeout.Get(timeout.DefaultAssertTimeout, p.config.Timeouts.Assert, p.test.Spec.Timeouts.Assert, p.step.TestStepSpec.Timeouts.Assert, to),
...
func (p *stepProcessor) commandOperation(ctx context.Context, exec v1alpha1.Command, to *metav1.Duration) operation {
	...
	return operation{
		timeout:         timeout.Get(timeout.DefaultExecTimeout, p.config.Timeouts.Exec, p.test.Spec.Timeouts.Exec, p.step.TestStepSpec.Timeouts.Exec, to),
...
func (p *stepProcessor) deleteOperation(ctx context.Context, op v1alpha1.Delete, to *metav1.Duration) (*operation, error) {
	...
	return &operation{
		timeout:         timeout.Get(timeout.DefaultDeleteTimeout, p.config.Timeouts.Delete, p.test.Spec.Timeouts.Delete, p.step.TestStepSpec.Timeouts.Delete, to),
...
func (p *stepProcessor) errorOperation(ctx context.Context, op v1alpha1.Error, to *metav1.Duration) ([]operation, error) {
	...
	for _, resource := range resources {
		ops = append(ops, operation{
			timeout:         timeout.Get(timeout.DefaultErrorTimeout, p.config.Timeouts.Error, p.test.Spec.Timeouts.Error, p.step.TestStepSpec.Timeouts.Error, to),
		...
		cleaner = func(obj unstructured.Unstructured, c client.Client) {
			...
			t.Cleanup(func() {
				operation := operation{
					...
					timeout:         timeout.Get(timeout.DefaultCleanupTimeout, p.config.Timeouts.Cleanup, p.test.Spec.Timeouts.Cleanup, p.step.TestStepSpec.Timeouts.Cleanup, nil),
