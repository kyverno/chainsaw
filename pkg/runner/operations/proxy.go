...
func (o proxyAction) Execute(ctx context.Context, tc enginecontext.TestContext) (outputs.Outputs, error) {
	...
	if tc, err := enginecontext.SetupContextAndBindings(tc, contextData); err != nil {
		...
		entrypoint, args, err := kubectl.Proxy(ctx, tc.Compilers(), client, tc.Bindings(), &o.op)
		if err != nil {
			return nil, err
		}
		op := opcommand.New(
...
func proxyOperation(basePath string, namespacer namespacer.Namespacer, op v1alpha1.Proxy) Operation {
	return proxyAction{
		basePath:   basePath,
